...
---------------------------------------------------------------------
select DATEADD(month,3, CURRENT_TIMESTAMP) as threemonth;
select DATEDIFF(day, CURRENT_TIMESTAMP,DATEADD(month,3,current_timestamp)) as diffdays;
select DATEDIFF(week, DATEFROMPARTS(1992,4,4), datefromparts(2011,9,16)) as diffweeks; 
select DATEADD(day,-22,CURRENT_TIMESTAMP) as firstday;
