...
Select 'The unit price for the ' + productname + ' is '+ Convert(nvarchar, unitprice) + '$.' As productdesc
...
Select orderdate, orderid, shippeddate, CoalEsce(shipregion, 'No region') As shipregion
From Sales.Orders
Where orderdate Between '20070401' And '20071130' And
DATEDIFF(day, orderdate , shippeddate) > 30;
...
Select Try_Convert(int, Replace(Replace(Replace(Replace(Replace(phone,'(', ''), ')',''),'-',''),' ', ''),'.','')) As phone
