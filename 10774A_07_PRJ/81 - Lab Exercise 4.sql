...
Select REPLACE(contactname, ',', ' ') As conctactname, Substring(contactname, Charindex(',', contactname)+1, Len(contactname) - Charindex(',', contactname)) As firstname
From Sales.Customers; 

----------------
--task 3

Select replace(contactname, ',','') as newcontactname,
...
Select custid, Format(custid, '\C00000')
From Sales.Customers;
...
---------------------------------------------------------------------
Select contactname,
	 Len(contactname) - Len(Replace(contactname, 'a', '')) As a_number
From Sales.Customers
...
SELECT
  c.contactname,
  LEN(c.contactname) - LEN(REPLACE(c.contactname, 'a', '')) AS numberofa
